...
組立とは逆の手順で、ProMicroカバー、OLED、ProMicro、M2x12mmスペーサー2つ、キースイッチの順に取外していきます。
...
画像の様に、M2x10mmのねじ2本にM2x4mmスペーサーをそれぞれ取付けます。スペーサーは回転させたときにねじが回らない位の強さで取付けられていればOKです。取付時にProMicroが邪魔な場合は取り外しておきます。(画像では取外しています。）
...
画像の様に、変換基板へはんだ付けしたピンヘッダ1x4を、メイン基板上のロープロファイルピンソケット1x4に、メイン基板のトッププレートを介して取付けます。
...
画像を参考に、トラックボールモジュールの配線を、メイン基板のトッププレートやトラックボールモジュール用ボトムプレートの穴へ通し、トラックボールモジュールをボトムプレートの上に置けるようにします。
...
(sekimenの場合、一番手っ取り早いのは、n2useのキーマップを使用してファームウェアの書込み、それをベースにキーマップの変更を行うことです。
