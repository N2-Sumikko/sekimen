...
**左キーボード**のProMicroとPC間をUSBケーブルで接続します。

sekimenは左キーボードのProMicroにUSBケーブルを接続しないと正しく動作しないので注意してください。
...
その後、画像のようにピンセットト等で、基板のキースイッチはんだ付け用の端子間をショートさせます。

(ピンセットではなく、押した状態のキースイッチを使用して動作確認を兼ねるのも良いかと思います。スイッチをはんだ付け済みの場合は単純にスイッチを押します。)
