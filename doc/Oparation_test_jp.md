...
ファームウェア書込み環境を構築し、ProMicroへファームウェアを書き込み、キーボードの入力動作を確認するまでの手順を記載しています。(2020/07/24時点)
...
(余談：以前は不要だったと記憶していますが、現在は--recurse-submodulesオプションが必要な様です。このオプションを使用しないとファームウェアの書込み時にエラーが発生します。)
...
# (右キーボードにOLEDを使用しない場合)ファームウェアの変更

　右キーボードにOLEDを使用しない場合、右キーボードへのファームウェア書込み時にファームウェアの変更が必要です。
...
変更が煩わしい場合は、左右キーボード両方にOLEDを使用する(または両方とも使用しない)ことをお勧めします。
...
(ピンセットではなく、押した状態のキースイッチを使用して動作確認を兼ねるのも良いかと思います。スイッチがはんだ付け済みの場合は単純に押します。)

![入力動作確認用](https://user-images.githubusercontent.com/54104281/88373551-ffffc580-cdd2-11ea-85f6-d4032195b3a9.png)

組立が正しく行われていれば、ショートさせた箇所に対応するキーのが入力されます。

これを左右基板の各端子、もしくはキースイッチに対して行います。

(レイヤキーの動作テストに関しては、例えばsekimenのデフォルトのキーマップでは、ADJUST1のレイヤキーを入力しながらqキーを押すと1が入力されるので、それらを利用して確認すると良いです。)

キーの入力に問題が無ければ、入力動作テストは完了です。
