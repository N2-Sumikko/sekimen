...
# 同梱品の確認
以下の部品が2セット揃っていることを確認します。(ただし、※がある部品は備考欄も確認してください。)

*   メイン基板

|名前|数|備考|
|:-|:-|:-|
|※メイン基板|1枚|左キーボード、右キーボード用で各1枚、合計2枚です。|
|トッププレート|1枚|PCB製です。左右共通です。|
|ボトムプレート|1枚|アクリル製です。左右共通です。|
|OLEDカバー|1枚|アクリル製です。左右共通です。|
|ダイオード|23個|SMDタイプです。|
|ピンヘッダ1x8|1個|拡張基板未使用時は余ります。|
|ピンソケット1x4 |1個||
|ピンソケット1x12|2個||
|ピンヘッダ1x12|2個|ProMicroに付属しています。|
|※ピンヘッダ1x4|1個|OLED用に合計1個付属しています。OLEDにはんだ付け済みの場合は付属しません。|
|細ピンヘッダ1x3|2個||
|ジャンパピン|2個|色はロットによって変わる場合があります。|
|リセットスイッチ|1個|色はロットによって変わる場合があります。|
|TRRSジャック|1個||
|※OLED|1個|左キーボード用のみ、合計1個付属しています。右手用はオプションです。|
|pro micro|1個||
|LEDテープ|1個||
|M2x7mmスペーサー|4個||
|M2x12mmスペーサー |2個||
...
|名前|数|備考|
|:-|:-|:-|
|※拡張基板|1枚|左キーボード、右キーボード用の拡張基板が1体になったものが合計1枚付属しています。|
|※トッププレート|1枚|PCB製です。左右キーボード用のトッププレートが1体になったものが合計1枚付属しています。|
|ボトムプレート|1枚|アクリル製です。左右共通です。|
|ダイオード|23個|SMDタイプ|
|ピンヘッダ1x8|1個||
|ジャンパピン|8個|色はロットによって変わる場合があります。|
|ゴム足|4個||
...
* Cherry MX互換キースイッチ46個(拡張基板使用時は66個)

* キーキャップ46個(拡張基板使用時は66個)

* (オプション)OLED(1個。ロゴを表示する場合、右キーボードに必要です。使用しなくてもキーボードは動作可能ですが、その際は右キーボード用のProMicroへのファームウェア書込時に一行変更が必要です。詳しくは後述します。）
...
* 瞬間接着剤(ピンヘッダやピンソケットの取付け時に、部品の樹脂部分に少量だけ塗布(金属部分にはつかないように!)し、部品の位置決めに使用しました。)

他にも、こちらのWebページも非常に参考になるかと思います。
https://salicylic-acid3.hatenablog.com/entry/2018/11/24/%E8%87%AA%E4%BD%9C%E3%82%AD%E3%83%BC%E3%83%9C%E3%83%BC%E3%83%89%E3%82%92%E4%BD%9C%E3%82%8B%E3%81%9F%E3%82%81%E3%81%AB%E5%BF%85%E8%A6%81%E3%81%AA%E3%82%82%E3%81%AE
...
出ていない時は、基板の穴からキースイッチの端子が出るように修正します。（キースイッチの取付け向きや端子の角度に問題が無いか、等）

# 四隅のキースイッチのはんだ付け

トッププレートの四隅へ取り付けたキースイッチをはんだ付けします。

先に残りのキースイッチを取付けても良いです。
...
写真のようにピンヘッダ1x8を基板に取付けます。(短い方が基板側です。後の手順で長い方にジャンパピンを取り付けます。間違えないようにしてください。)
...
取付後、ピンヘッダの端子8つをはんだ付けします。(はんだ付け完了後の画像を貼るつもりでしたが、先走ってキースイッチを取付けた画像を貼っています。すみません。)
...
まずは写真のようにベース基板へピンヘッダ1x8を取付けます。(こちらも短い方が基板側ですので注意してください。)
...
画像のように、ベース基板と拡張基板のピンヘッダ1x8の隣り合っている端子間を繋ぐように、ジャンパピン8つを取付けます。

ベース基板と拡張基板の位置があっていない場合は微調整します。(ボトムプレートが仮止め状態であれば、若干の位置調整が可能です。)
...
(ベース基板の入力動作テストは済んでいると思いますので、ここでは、拡張基板部分の入力動作を重点的に確認します。)
