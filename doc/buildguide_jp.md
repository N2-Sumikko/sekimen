...
* OLED(最大2個。1個はロゴ用なので使わなくても支障ありません。使わない場合ファームウェアの変更が必要です。）
...
* はんだ付けが難しいと感じたら、出来る限りフラックス使用後にはんだ付けするようにしてください。(特にLEDテープのはんだ付けは、特別な理由が無ければ最初からフラックスを塗ることを推奨します。)
...
この作業を右手、左手キーボードで各23個、合計46個分行います。
...
取付後、リセットスイッチの端子2つをはんだ付けします。必要なはんだの量はTRRSジャックと比べると少し多めです。
...
キーボード間の通信をシリアル通信で行う場合は画像のように取り付けます。

ジャンパピンの正しい位置にピンヘッダの端子が入っていることを確認してください。

I2C通信時の取付け方法を示す画像は後程追加予定です。(I2C通信可能なように作りましたが、実はまだ試していません。あまり需要はないと思っています…。)
...
(もし組立てに問題があった時に、キースイッチが原因かそれ以外かを切り分けしやすいためです。)
...
LEDテープの端子にははんだが乗りにくいので、フラックスを塗ってからのはんだ付けを推奨します。(私はフラックス無しではんだジャンパ出来ない程度の能力です。)
...
画像のように、アクリルのボトムプレートをM2x7mmスペーサー4つへM2x4mmねじ4本でねじ止めします。

拡張基板を使用する場合は、この手順は飛ばして構いません。
...
写真のように拡張基板がつながっている場合は、ニッパー等で切り離します。
...
取付後、ピンヘッダの端子8つをはんだ付けします。(写真は先走ってキースイッチを取付けていますが、ここではピンヘッダ1x8のはんだ付け部分のみ見てください。すみません。)
...
拡張基板のトッププレートが左右一体になっている場合は、拡張基板と同様の手順で切り離します。

写真のように、拡張基板のトッププレートの四隅へキースイッチを取付けます。
...
四隅のキースイッチの端子が基板の穴から正しく出ていることを確認したら、キースイッチを基板からなるべく浮かないようにはんだ付けします。
...
まずは写真のようにベース基板へピンヘッダ1x8を取付けます。(細い方が基板側ですので注意してください。)

取付け時にキースイッチが邪魔な場合は、邪魔なキースイッチをいったん取外します。
...
その後、キースイッチやダイオード、ピンヘッダのはんだ付けに抜けが無いことを確認しておきます。
...
写真のようにボトムプレート2枚をM2x7mmスペーサー6つへM2x4㎜ねじ6本で仮止め(部品が脱落しない程度の強さでねじ止め)します。
...
仮止め後、写真のようにボトムプレートへゴム足8つを取付けます。(すでに取付けられているゴム足の取付は飛ばします。)
...
写真のように、ベース基板と拡張基板のピンヘッダ1x8の各端子を繋ぐように、ジャンパピン8つを取付けます。
...
ジャンパピンは1つずつ、奥まで取り付けられていることを確認しながら取付けます。端子の位置が写真のようになっていれば大丈夫です。
...
2つのキーボード間をTRRSケーブルで接続し、左手側のキーボードのPro_microとPC間をUSBケーブルで繋ぎ、キーボードの動作確認します。
