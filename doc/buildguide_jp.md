...
* (オプション)OLED(1個。ロゴを表示するために右キーボードに必要です。左キーボードだけにOLEDを使用した状態でもキーボードを動作させることは可能ですが、その場合は右キーボードのProMicroへファームウェア書込時にファームウェアの変更が必要です。詳しくは入力動作テストの項目に記載します。）

* (オプション)不要なリード部品のリード線等(拡張基板使用時のキーボードの強度を上げたい場合、メイン基板と拡張基板のトッププレート間のはんだ付けに使用します。) 
