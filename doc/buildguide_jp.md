...
* (オプション)OLED(1個。ロゴを表示する場合、右キーボードに必要です。左キーボードだけにOLEDを使用した状態でもキーボードは動作可能ですが、その場合は右キーボード用のProMicroへのファームウェア書込時にファームウェアの変更が必要です。詳しくは後述します。）

* (オプション)リード部品の破片等(拡張基板使用時のキーボードの強度を上げたい場合、メイン基板と拡張基板のトッププレート間のはんだ付けに使用します。) 
...
* 瞬間接着剤(ピンヘッダやピンソケットの取付け時に、部品の樹脂部分に少量だけ塗布(金属部分にはつかないように注意です!)し、部品の位置決めに使用しました。)

他にも、こちらのWebページも非常に参考になるかと思います。(ページの作成者様にこの場を借りて感謝致します。)
...
もしすべてのピンのはんだ付け後にピンソケットの高さや角度が悪いことに気づいた場合は、使用上問題無い場合もありますので先の手順に進んでみてください。ダメな場合はピンソケット1x12へのProMicro取付けあたりで判明すると思います。
...
I2C通信時の取付け方法の画像は後程追加予定です。(I2C通信可能なように作りましたが、使用したい部品が入手できておらず、まだ試していません…。)
...
入力動作テストに問題が無ければ、２２メイン基板の組立てはこれで完了です。おつかれさまでした。
...
メイン基板と拡張基板の接続強度を上げたい場合、トッププレートの画像の赤枠で囲んだ部分を、不要なリード部品のリード線等で繋いではんだ付けします。

はんだ付け可能な場所は3箇所ありますが、赤枠で囲んでいない箇所は予備です。
